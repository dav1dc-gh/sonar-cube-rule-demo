...
Usage:
    ...
    python3 scripts/validate-rules.py --write-index # (Re)generate rules/INDEX.md
"""
...
def write_index(project_root: Path) -> Path:
    """Generate rules/INDEX.md from rule files on disk. Returns the path written."""
    rules_dir = project_root / "rules"
    by_category: dict[str, list[dict]] = {cat: [] for cat, _ in INDEX_CATEGORIES}

    for filepath in discover_rule_files(str(project_root)):
    ...
    lines.append("")
    lines.append("<!-- AUTO-GENERATED by scripts/validate-rules.py --write-index. Do not edit by hand. -->")
    lines.append("")
    total = sum(len(v) for v in by_category.values())
    lines.append(f"Total rules: **{total}**")
    lines.append("")

    for cat_dir, cat_label in INDEX_CATEGORIES:
        rules = sorted(by_category[cat_dir], key=lambda r: r["key"])
        lines.append(f"## {cat_label} ({len(rules)})")
        lines.append("")
        if not rules:
            lines.append("_No rules in this category._")
            lines.append("")
            continue
        lines.append("| Key | Name | Severity | Type | Description |")
        lines.append("|-----|------|----------|------|-------------|")
        for r in rules:
            ...
            lines.append(
                f"| [`{r['key']}`]({r['path']}) | {r['name']} | {r['severity']} | {r['type']} | {desc} |"
            )
        lines.append("")

    out = rules_dir / "INDEX.md"
...
def main():
    ...
    write_index_flag = "--write-index" in sys.argv
